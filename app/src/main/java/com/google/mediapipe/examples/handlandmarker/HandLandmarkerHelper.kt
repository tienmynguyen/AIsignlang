...
class HandLandmarkerHelper(
    ...
    fun detectVideoFile(
        ...
        return if (didErrorOccurred) {
            ...
            ResultBundle(resultList, inferenceTimePerFrameMs, height, width, 0)
        }
    }
    ...
    private fun preProcessLandmark(landmarkList: List<Float>): List<Float> {
        if (landmarkList.isEmpty()) {
            Log.w(TAG, "Landmark list is empty. Skipping pre-processing.")
            return emptyList()
        }

        val tempLandmarkList = landmarkList.toMutableList()

        // Chuyển đổi sang tọa độ tương đối
        val baseX = tempLandmarkList[0]
        val baseY = tempLandmarkList[1]
        for (i in tempLandmarkList.indices step 2) {
            tempLandmarkList[i] = tempLandmarkList[i] - baseX
            tempLandmarkList[i + 1] = tempLandmarkList[i + 1] - baseY
        }

        // Chuẩn hóa
        val maxValue = tempLandmarkList.maxOfOrNull { abs(it) } ?: 1f
        return tempLandmarkList.map { it / maxValue }
    }
    ...
    private fun returnLivestreamResult(
        ...
        // Tạo danh sách keypoint
        val keypointList = mutableListOf<Float>()
        for (landmark in result.landmarks()) {
            for (normalizedLandmark in landmark) {
                val landmarkX = minOf((normalizedLandmark.x() * imageWidth).toInt(), imageWidth - 1)
                val landmarkY = minOf((normalizedLandmark.y() * imageHeight).toInt(), imageHeight - 1)
                keypointList.add(landmarkX.toFloat())
                keypointList.add(landmarkY.toFloat())
            }
        }

        val processedLandmarkList = preProcessLandmark(keypointList)
        val classifier = KeyPointClassifier(context)
        val resultIndex = classifier.classify(processedLandmarkList)
        Log.d(TAG, "Keypoint list: $resultIndex")
        ...
        handLandmarkerHelperListener?.onResults(
            ResultBundle(
                ...
                input.height,
                input.width,
                resultIndex // Thêm resultIndex vào ResultBundle
            )
        )
    }
    ...
    data class ResultBundle(
        ...
        val resultIndex: Int // Thêm resultIndex vào ResultBundle
    )
